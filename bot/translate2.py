...
class Translate:
    ...
    def translate(self, text):
        tr_text = "|".join(text)
        try:
            return requests.post("https://www.m-translate.it/translate", data={
                "translate_to": "ru",
                "translate_from": "pl",
                "text": tr_text}).json()['translate'].split("|")
    ...
    def yandex_translate_item(self, item):
        item["discr"] = re.sub(r"\d+\.\d+\.\d+", "", item["discr"])
        item["details"] = re.sub(r"\d+\.\d+\.\d+", "", item["details"])

        cats = ["name", "surname", "discr", "dimensions", "instructions", "material", "details", "rules"]

        tr_text = []
        for cat in cats:
            tr_text.append(item[cat])

        tr_text = self.translate(tr_text)
        for t, cat in zip(tr_text, cats):
            item[cat] = t
        item["tags"] = self.translate(item["tags"])
